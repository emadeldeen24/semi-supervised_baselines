...
def jitter(x, sigma=0.8):
    ...
    x_out = x + np.random.normal(loc=0., scale=sigma, size=x.shape)
...
def permutation(x, max_segments=5, seg_mode="random"):
    ...
    return torch.from_numpy(ret).squeeze(0)
...
def cutPF(ts, perc=.5):
    ...
    new_ts = ts.copy()
    t=int(perc*seq_len)
    return torch.from_numpy(new_ts[:t, ...]), torch.from_numpy(new_ts[t:, ...])


def DataTransform(sample):

    weak_aug = scaling(sample)
    strong_aug = jitter(permutation(sample)) #.squeeze(1)

    return weak_aug, strong_aug

def DataTransform_SemiTime(sample):
    # shapre should be [1024,1]
    if 1 in sample.shape and sample.shape[0] == 1:
    ...
    return t1.transpose(0, 1), t2.transpose(0, 1), t3.transpose(0, 1), t4.transpose(0, 1)