...
class trainer(object):
    def __init__(self, args):
        ...
        # Exp Description
        self.run_description = args.ssl_method
    ...
    def train(self):
        ...
        # fixing random seed
        fix_randomness(int(self.seed))

        # Logging
        self.logger, self.scenario_log_dir = starting_logs(self.dataset, self.ssl_method, self.backbone,
                                                           self.train_mode, self.exp_log_dir, self.seed)

        # Load data
        self.load_data(self.dataset)

        # get algorithm
        backbone_fe = get_network_class(f"cnn1d_fe")
        ...
        if self.train_mode == "ssl":
        ...
        elif "ft" in self.train_mode or  "lc" in self.train_mode:
            backbone_fe = backbone_fe(self.dataset_configs)
            backbone_temporal = backbone_temporal(self.hparams)
            ...
            if self.train_mode == "lc":
                for param in backbone_fe.parameters():
        ...
        for epoch in range(1, self.hparams["num_epochs"] + 1):
            ...
            if self.train_mode == "ssl" and self.ssl_method in ["FixMatch", "SemiTime", "MeanTeacher", "DivideMix"]:
                joint_loaders = enumerate(zip(self.train_dl, self.labeled_few_shot_dl))
                for step, (data, fl_data) in joint_loaders:
                    data = to_device(data, self.device)
                    fl_data = to_device(fl_data, self.device)
                    losses, model = algorithm.update(data, fl_data)
        ...
        if self.train_mode != "ssl":
            ...
            self.calc_results_per_run()
            self.logger.debug(f'Acc:{self.acc:2.4f} \t F1:{self.f1:2.4f}')

            self.save_results()
            _save_metrics(self.pred_labels, self.true_labels, self.scenario_log_dir, self.home_path)


        # logging metrics at the last fold:
        # if self.seed == "2" and self.train_mode != "ssl":  # change "4" if you used different k-fold settings.
    ...
    def get_configs(self):
        if self.dataset not in ["EEG", "HAR", "Epilepsy"]:
            dataset_class = get_dataset_class("ucr")
        else:
    ...
    def load_data(self, data_type):
        ...
        output = data_generator(self.data_path, self.data_percentage,
                                                     self.dataset_configs, self.hparams, self.train_mode,
                                                     self.ssl_method, self.augmentation, self.oversample)

        if self.train_mode == "ssl" and self.ssl_method in ["FixMatch", "SemiTime", "MeanTeacher", "DivideMix"]:
            self.train_dl, self.val_dl, self.test_dl, num_classes, self.labeled_few_shot_dl = output
        else:
            self.train_dl, self.val_dl, self.test_dl, num_classes = output

        if self.dataset not in ["isruc"]:
            self.dataset_configs.num_classes = num_classes
    ...
    def calc_results_per_run(self):
        self.acc, self.f1 = _calc_metrics(self.pred_labels, self.true_labels)
