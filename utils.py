...
def starting_logs(data_type, ssl_method, backbone, train_mode, exp_log_dir, fold_id):
    ...
    logger.debug(f'Model:  {backbone}')
    logger.debug(f'Seed: {fold_id}')
    logger.debug("=" * 45)
...
def _save_metrics(pred_labels, true_labels, log_dir, home_path):
    pred_labels = np.array(pred_labels).astype(int)
    true_labels = np.array(true_labels).astype(int)
